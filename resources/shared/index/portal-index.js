function splash_login() {
    window.addEventListener("message", function (event) {
        if (window.portalEnv.authclient.startsWith(event.origin) && event.data === 'login_success') {
            ...
            if (splash_login) {
                splash_login.src = window.portalEnv.authclient;
            }
        }
    }, false);
    document.addEventListener('DOMContentLoaded', function () {
        document.getElementsByTagName('body')[0].insertAdjacentHTML('beforeend',
            '\n' +
            '<div id="splash">\n' +
            '    <div class=\"splash-background\" style=\"background-image: url(' + window.portalEnv.splashLoginBackground + ');">\n' +
            ...
            '                </div>\n' +
            '                <iframe allowtransparency="true" frameborder="0" id="splash-login" style="width:100%;height:100%" src="' + window.portalEnv.authclient + '/"></iframe>\n' +
            '            </div>\n' +
            '        </div>\n' +
            '    </div>\n' +
            '</div>\n'
